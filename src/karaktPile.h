...
void saveKarakt_to_KMem(byte place)
{
    ...
    // pas très safe d'utiliser une variable générale de cette façon, mais plus rapide que 
    // de mettre cette valeur en paramètre
    ...
    econome_int_saveToKMem(place, 133, FunkyVelValueMIDIPlayer);
    econome_bool_saveToKMem(place, 134, complexSqueeze);

    econome_int_saveToKMem(place, 135, iVLVDelta); /// DEBUG LONG !!!!
    econome_int_saveToKMem(place, 136, iDureeDelta); /// DEBUG LONG !!!!
    econome_int_saveToKMem(place, 137, imididouble); /// DEBUG LONG !!!!
    econome_int_saveToKMem(place, 138, iShiftDelta);
    econome_int_saveToKMem(place, 139, iShiftDuree);
    econome_int_saveToKMem(place, 140, iDureeMask);
    econome_int_saveToKMem(place, 141, iMask2);

    econome_int_saveToKMem(place, 142, CutOffDureeDelta);
    econome_int_saveToKMem(place, 143, BringToDureeDelta);
    //  float RatioDeltaToMsec = 4.0; TODO il faut le calculer !

    econome_int_saveToKMem(place, 144, NumRatioDeltaToMsec);
    econome_int_saveToKMem(place, 145, DenomRatioDeltaToMsec);

    econome_int_saveToKMem(place, 146, transposeMIDIPlayer);

    econome_int_saveToKMem(place,147, loCutMIDI);
   econome_int_saveToKMem(place,148, loBringToMIDI);
    econome_int_saveToKMem(place,149, hiCutMIDI);
   econome_int_saveToKMem(place,150,  hiBringToMIDI);
    ...
    for (int j = 0; j <= place; j++)
    {
        ...
        do
        {
           ...
           // Serial.print(" ");
            ...
            // DEBUG libérer les lignes ci-dessous et s'assurer que les save et load sont OK pour LONG 
            // long_loadFromKarakt(136, &iDureeDelta);
            // long_loadFromKarakt(135, &iVLVDelta);

            // long_loadFromKarakt(137, &imididouble);
