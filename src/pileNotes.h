...
void putNoteBinNoteA(pileNote &A, pileNote &B)
{
    ...
    A.noteReallyPlayed = B.noteReallyPlayed;
    A.channelReallyPlayed = B.channelReallyPlayed ; 
    A.thereIsAnEnd = B.thereIsAnEnd;
    A.killnote = B.killnote;

    A.pinchCH = B.pinchCH;
                    A.pinchCH_size = B.pinchCH_size;
                                         A.pinchCC = B.pinchCC;
                                                         A.pinchCC_size = B.pinchCC_size;
                                                                              A.pinchValue = B.pinchValue;
        ...
        do
        {
            ...
            if ((*pilePtr).timeToPlayON <= nowMoment)
            {
                ...
                // if ( true == true )
                {
                    ...
                    // TODO DEBUG BLACK08 : .channel ou .channelReallyPlayed ? 
...
int nowOmitEndValue = getValueStraightOrFromSequence(omitEndValue[(*pilePtr).layer], 0 );
if (omitEnd[(*pilePtr).layer] == true && (*pilePtr).type == 0x80 && (int)random(100) < nowOmitEndValue)
{
...
int getRedirectValue_from_NoteAChannelBinSoftPassPiles(byte n, byte CH)
{
    ...
    { // va parcourir la mélodie pointée ( Layer 1 à 4 : potVal3 )
        ...
        if (tempNote.note == n && tempNote.channel == CH)
        {
            return tempNote.noteReallyPlayed ;
        }
    }
