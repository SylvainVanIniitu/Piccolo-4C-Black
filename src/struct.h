...
struct pileNote
{
    ...
    byte noteReallyPlayed;
    byte channelReallyPlayed ; // BLACK08 utile à partir du moment où on joue avec Volca à 16 CH ! 
    ...
    // BLACK08 je ne suis pas certain que ce soit pertinent et nécessaire à vrai dire
    // delete ?   attention à la sauvegarde/load des .txt qui en tiennent compte
    // compatibilité dans les 2 sens ! 
    ...
    byte type;  // 0x90 = noteON   0x80 = OFF    0xB0 = CC    0x50 ACCENT ???    0x01 PINCH MULTI 
    
    byte layer; // je me demande si c'est utile - oui ça l'est, car certaines modifs sont liées au layer de la note !

// essai 22C01 pour Pinch Multi : à terme mettre dans une autre pile ? 
    int8_t pinchCH;
        int8_t pinchCH_size;
            int8_t pinchCC;
                int8_t pinchCC_size;
