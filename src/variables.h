...
// Juillet 2022 : NARAYANA's COWS

// NARAYANA's COWS

const int sizeTroupeauMax = 1000;
int Parents[sizeTroupeauMax][3] = {0};
int Enfants[sizeTroupeauMax][3] = {0};
int initialPitch = 64;           // should be 64 : E
int tailleScale = 2;       // should be 2
int scale[7] = {1, 3}; // should be 1,3
int maturite = 4;                // should be 4
int menopause = 1000;            // should be infinite
int mort = 1000;                 // should be infinite
int ageInitialPremiereVache = 4; // should be 4
int taillePortee = 1;            // nombre de veaux qui naissent en même temps : should be 1
int mortaliteFortuite = 0;       // en % les veaux et vaches qui meurrent aléatoirement : should be 0

long Naranaya_BPM = 200;
long dureeVache = (60 * 1000) / Naranaya_BPM;
long ratioVeau = 2 ; 
long dureeVeau = dureeVache / ratioVeau ; // should be 2
int narayanaMode = 1 ; // 1 = note 2 =  channel 3 = channel+note 

// 22C01 An Vert

int transposeMIDIPlayer = 0 ;  // permettra de transposer en direct le jeu du midi player 
...
int acidVelocityMin = 80 ;
int acidVelocityMax = 120 ; 
int acidNbDivide = 3 ; // nombre de division dans u nburst, typiquement 3, un triplet quoi 

int gabbUltrinoSequence = 0 ;
bool premonitionON[nbLayer] = { false } ; 
int premonitionValue[nbLayer] = {0} ; 
int dropPourPremonition[nbLayer] = {100,100,100,100} ;
int premonitionRandomAppear[nbLayer] = { 100, 100, 100, 100} ;
int rndStepzCH = 0 ;
int rndStepzCC = 0 ;

    // BLACK09
    int keyboardMode = 0;      // = le mode note par défaut
bool DMXautoGenerate = false ; // qd on, envoie du DMX à chaque note
int decalagemSwing = 0 ; 
...
int seqPostpone[nbLayer] = {0}; // indique la séquence que le sequential postpone va utiliser pour le layer

// keyboard STEPZ 2C30 17 septembre 2020 comment ai-je pu avoir besoin de tant de temps pour y penser ?
// bool keyboardStepping = false ;  //BLACK09
byte startingKbStep = 48; // pour keystep habituel - commencer avec un C ! 
...
bool verboseSerial_OrderGiven = true ; // ajout 32C02 30 juillet 2022 pour uQitll 
...
bool trackerKaraktActif[nbLayer] = {false, false, false, false};
int trackerKaraktSeq[nbLayer] = {0, 0, 0, 0}; // définit la séquence qui est liée au tracker
...
unsigned int nbMaxDeNotesParLayer[nbLayer] = {1000, 1000, 1000, 1000};
...
const int duree130bpm =  (( 60*1000) / 130 ) * 8 ;
unsigned long layerLength[4 + 64 + 4 * 4 + 10] = {duree130bpm, 2 * duree130bpm, 3 * duree130bpm, 4 * duree130bpm }; // indiquera la durée de chaque layer, en msec    // si on veut des boucles de plus de 32 secondes on doit utiliser LONG pour les 3 variables suivante
...
// BLACK 10 : modifié les 3 valeurs ci-dessous de LONG en INT
int iVLVDelta = 0;         // K SL
int iDureeDelta = 0;       // K SL
int imididouble = 0; // K SL

int iShiftDelta = 8; // K SL
int iShiftDuree = 7; // K SL
int iDureeMask = B01111111; // K SL
int iMask2 = B10000000;     // K SL

int CutOffDureeDelta = 10000; // NOTE **** devra peut-être être changée pour ne pas avoir de cutoff qd on veut perfection
                             // K SL
int BringToDureeDelta = 0;   // K SL
// float RatioDeltaToMsec = 4.0;
int NumRatioDeltaToMsec = 100; // K SL
int DenomRatioDeltaToMsec = 100; // K SL
