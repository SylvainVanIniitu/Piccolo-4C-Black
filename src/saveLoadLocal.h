...
void long_loadFromKarakt(int number, long *here)
{
    if (karaktNumber == number)
    {
        *here = (long)karaktValue;
    }
}
               ...
               bool saveNoteRedirect = true, bool saveLFOs = true, bool saveKarakt = true)
{
    ...
    //  myFile = SDcard.open(fileString, FILE_WRITE);

    myFile.close();
    if (SDcard.remove(fileString))
    {
        oledWriteStringAtXY("REMOVED   ", 0, 3);
     //   delay(5000);
    }
    else
    {
        oledWriteStringAtXY("NO REMOVED", 0, 3);
      //  delay(5000);
    }
    ...
    if (myFile.isOpen())
    {
        oledWriteStringAtXY("OPEN OK     ", 0, 3);
      //  delay(5000);
    }
    else
    {
        oledWriteStringAtXY("OPEN PROBLEM", 0, 3);
     //   delay(5000);
    }
    ...
    if (myFile.close())
    {
        oledWriteStringAtXY("CLOSE OK     ", 0, 3);
       // delay(5000);
    }
    else
    {
        oledWriteStringAtXY("CLOSE PROBELM     ", 0, 3);
       // delay(5000);
    }
...
String resumeLoad = "" ; // servira à afficher le résultat du load 
...
resumeLoad = ""; 
    ...
    myFile = SDcard.open(name, FILE_READ);

    // Serial.println("opened ");
    // erial.println(name);

    // BLACK08 : pq on met le nom dans Name ? qui n'appartient pas à la fonction ?
    ...
    { // Debut lecture
        ...
        // Serial.println("1"+ligne);
        cleanLigne(ligne);

        // Serial.println("2"+ligne);
        check = myFile.peek();
        // Serial.println("3"+ligne);

        //  buffer_clearLine(7);
        buffer_clearLine(2);
        // Serial.println("4"+ligne);
        buffer_and_HARD_setCursor(0, 2);
        // Serial.println("5"+ligne);

        int limitUP = 12;
        if (ligne.length() < 12)
        {
            limitUP = ligne.length();
        }
        //  Serial.print(limitUP);
        if (limitUP != 0)
        {
            buffer_and_HARD_print(ligne.substring(0, limitUP));
        }
        //  Serial.println("6"+ligne);
        delay(watchDelay);
        // Serial.println("7"+ligne);
...
resumeLoad = resumeLoad + String(rememberLayer) + "=" + String(compteurNote) + " " ; 
compteurNote = 0 ; // 22C01 
                                        ...
                                        while (keepon == true)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                */
                                                ...
                                                int_loadFromKarakt(133, &FunkyVelValueMIDIPlayer);
                                                bool_loadFromKarakt(134, &complexSqueeze);

                                                 int_loadFromKarakt(135, &iVLVDelta);
                                                 int_loadFromKarakt(136, &iDureeDelta);
                                                 int_loadFromKarakt(137, &imididouble);
                                                 int_loadFromKarakt(138, &iShiftDelta);
                                                 int_loadFromKarakt(139, &iShiftDuree);
                                                 int_loadFromKarakt(140, &iDureeMask);
                                                 int_loadFromKarakt(141, &iMask2);

                                                 int_loadFromKarakt(142, &CutOffDureeDelta);
                                                 int_loadFromKarakt(143, &BringToDureeDelta);
                                                //  float RatioDeltaToMsec = 4.0; TODO il faut le calculer !

                                                 int_loadFromKarakt(144, &NumRatioDeltaToMsec);
                                                 int_loadFromKarakt(145, &DenomRatioDeltaToMsec);

                                                 int_loadFromKarakt(146, &transposeMIDIPlayer );

                                                 int_loadFromKarakt(147, &loCutMIDI );
                                                 int_loadFromKarakt(148, &loBringToMIDI );
                                                 int_loadFromKarakt(149, &hiCutMIDI );
                                                 int_loadFromKarakt(150, &hiBringToMIDI );
...
bool complexSqueeze = false;   // pas sauvé dans Karakt

long iVLVDelta = 0;         // pas sauvé dans Karakt
long iDureeDelta = 0;
long imididouble = 0;
byte iShiftDelta = 8;
byte iShiftDuree = 7;
...
// MIDI PLAY
                int_printToKarakt(133, FunkyVelValueMIDIPlayer, "funkyVelMidiPlayer");
                bool_printToKarakt(134, complexSqueeze, "complexSqueeze" ); 

                int_printToKarakt(135, iVLVDelta , "iVLVDelta"); 
                int_printToKarakt(136, iDureeDelta , "iDureeDelta");
                int_printToKarakt(137, imididouble , "imidiDouble");
                int_printToKarakt(138,  iShiftDelta , "ishiftDelta");
               int_printToKarakt(139, iShiftDuree , "ishiftDuree"); 
                int_printToKarakt(140, iDureeMask , "idureeMask"); 
               int_printToKarakt(141, iMask2 , "imask2"); 

                int_printToKarakt(142,  CutOffDureeDelta, "icutOff");
                int_printToKarakt(143, BringToDureeDelta , "iBringTo"); 
              //  float RatioDeltaToMsec = 4.0; TODO il faut le calculer ! 

                int_printToKarakt(144, NumRatioDeltaToMsec, "NumRatio speed"); 
                int_printToKarakt(145, DenomRatioDeltaToMsec , "denomRatio speed"); 

// 22C01 An Vert

                int_printToKarakt(146, transposeMIDIPlayer,"transpose MIDI player");

                int_printToKarakt(147,  loCutMIDI, "lo VLV cut"); 
                 int_printToKarakt(148, loBringToMIDI, "lo bring to"); 
                 int_printToKarakt(149, hiCutMIDI, "hi VLV cut"); 
                 int_printToKarakt(150, hiBringToMIDI, "hi bring to") ; 
        ...
        for (unsigned int j = 0; j < karakt_nbTotalPiles; j++)
        {
            ...
            // parcourir la chaîne            // charger value, number, layber
            ...
            else
            {
                ...
                do
                {
                    ...
                    // MIDI PLAY
                    fromKMem_int_printToKarakt(133, FunkyVelValueMIDIPlayer, "funkyVelMidiPlayer");
                    fromKMem_bool_printToKarakt(134, complexSqueeze, "complexSqueeze");

                    fromKMem_int_printToKarakt(135, iVLVDelta, "iVLVDelta");
                    fromKMem_int_printToKarakt(136, iDureeDelta, "iDureeDelta");
                    fromKMem_int_printToKarakt(137, imididouble, "imidiDouble");
                    fromKMem_int_printToKarakt(138, iShiftDelta, "ishiftDelta");
                    fromKMem_int_printToKarakt(139, iShiftDuree, "ishiftDuree");
                    fromKMem_int_printToKarakt(140, iDureeMask, "idureeMask");
                    fromKMem_int_printToKarakt(141, iMask2, "imask2");

                    fromKMem_int_printToKarakt(142, CutOffDureeDelta, "icutOff");
                    fromKMem_int_printToKarakt(143, BringToDureeDelta, "iBringTo");
                    //  float RatioDeltaToMsec = 4.0; TODO il faut le calculer !

                    fromKMem_int_printToKarakt(144, NumRatioDeltaToMsec, "NumRatio speed");
                    fromKMem_int_printToKarakt(145, DenomRatioDeltaToMsec, "denomRatio speed");

                    fromKMem_int_printToKarakt(146, transposeMIDIPlayer, "transpose MIDI player");

                    fromKMem_int_printToKarakt(147, loCutMIDI, "lo VLV cut");
                    fromKMem_int_printToKarakt(148, loBringToMIDI, "lo bring to");
                    fromKMem_int_printToKarakt(149, hiCutMIDI, "hi VLV cut");
                    fromKMem_int_printToKarakt(150, hiBringToMIDI, "hi bring to");
