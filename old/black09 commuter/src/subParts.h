...
// BLACK06    Serial3.begin(31250);
    ...
    PiccoloVersion = F("Piccolo Black08"); // petit subterfuge pour ne rien laisser en mémoire vive
    // BLACK08 euh... du coup ça l'y charge, non ?
    PiccoloDate = F("30.7.2021");
        ...
        else
        {
            ...
            else if (tapAccou == true)
            {
                ...
                for (byte i = 0; i < nbLayer; i++)
                {
                    ...
                    oldlayerLength = layerLength[i];
                    if ( oldlayerLength == 0 ) { oldlayerLength = 1 ; }
...
// le softwareEncoderLeft, et les valeurs du right 
        ...
        if (myButtonPad2.risingEdge())
        {
            jumpToTransfo(228); // select SEQ sequence 
        }
...
void StepzButtonPushed_LayerA_NoteB_ButtonC(int i, byte noteToCreate, byte ButtonPushed)
{
    ...
    for (unsigned k = 0; k < nbSubStepz; k++)
    {
        ...
        else
        {
            ...
            case 0:
            {
                tempNote2.endNote = (tempNote2.startNote + duree1subStep - (unsigned long) 3) % layerLength[i];
            }
            break;

            case 1:
            {
                tempNote2.endNote = (tempNote2.startNote + ((unsigned long)nbSubStepz * duree1subStep) - 3) % layerLength[i];
            }
            break;
