...
*/
    ...
    myTimerScreen.begin(updateScreen, 5000);               
    // 15000 = 15 millisecondes, parfait pour la persistance rétinienne
...
*/
    ...
    if (a != oldHardwareRight)
    {
        ...
        if (softwareValueEncoderRight < minimumSoftwareValueEncoderRight && loopingValueRight == false )
        {
            softwareValueEncoderRight = minimumSoftwareValueEncoderRight;
            stepSizeRight = 1;
        }

        if (softwareValueEncoderRight > maximumSoftwareValueEncoderRight && loopingValueRight == false)
        {
            softwareValueEncoderRight = maximumSoftwareValueEncoderRight;
            stepSizeRight = 1;
        }

        if (softwareValueEncoderRight < minimumSoftwareValueEncoderRight && loopingValueRight == true)
        {
            softwareValueEncoderRight = maximumSoftwareValueEncoderRight;
            stepSizeRight = 1;
        }

        if (softwareValueEncoderRight > maximumSoftwareValueEncoderRight && loopingValueRight == true )
        {
            softwareValueEncoderRight = minimumSoftwareValueEncoderRight;
            stepSizeRight = 1;
        }
...
maximumSoftwareValueEncoderRight = Transfo[softwareValueEncoderLeft][2]; 
    ...
    if (softwareValueEncoderLeft < minimumSoftwareValueEncoderLeft && loopingValueLeft == false)
{
    softwareValueEncoderLeft = minimumSoftwareValueEncoderLeft;
    stepSizeLeft = 1;
        }

        if (softwareValueEncoderLeft > maximumSoftwareValueEncoderLeft && loopingValueLeft == false)
        {
            softwareValueEncoderLeft = maximumSoftwareValueEncoderLeft;
            stepSizeLeft = 1;
        }

        if (softwareValueEncoderLeft < minimumSoftwareValueEncoderLeft && loopingValueLeft == true )
            {
                softwareValueEncoderLeft = maximumSoftwareValueEncoderLeft;
                stepSizeLeft = 1;
            }

        if (softwareValueEncoderLeft > maximumSoftwareValueEncoderLeft && loopingValueLeft == true )
            {
                softwareValueEncoderLeft = minimumSoftwareValueEncoderLeft;
                stepSizeLeft = 1;
            }
 ...
 //   writeStringAtXY(String(softwareValueEncoderLeft) + " ", 7, 1);
    ...
    if (a != oldHardwareUp)
    {
        ...
        if (softwareValueEncoderUp < minimumSoftwareValueEncoderUp && loopingValueUp == false)
        {
            softwareValueEncoderUp = minimumSoftwareValueEncoderUp;
            stepSizeUp = 1;
        }

        if (softwareValueEncoderUp > maximumSoftwareValueEncoderUp && loopingValueUp == false)
        {
            softwareValueEncoderUp = maximumSoftwareValueEncoderUp;
            stepSizeUp = 1;
        }

        if (softwareValueEncoderUp < minimumSoftwareValueEncoderUp && loopingValueUp == true )
        {
            softwareValueEncoderUp = maximumSoftwareValueEncoderUp;
            stepSizeUp = 1;
        }

        if (softwareValueEncoderUp > maximumSoftwareValueEncoderUp && loopingValueUp == true )
        {
            softwareValueEncoderUp = minimumSoftwareValueEncoderUp;
            stepSizeUp = 1;
        }
...
// Black07 uniformiser la dénomination des Checks : Check (type) Button(s)
    ...
    */
    ...
    // on va donc savoir de combien il a bougé durant le temps spécifié par timeToCheckEncoder ( initialement 1 seconde )
    {
        ...
        {
            stepSizeRight = stepSizeRight / multiplyRatioRight;
            // printRot2(String(stepSizeLeft), 10, 400, 7); // juste pour faire comprendre qu'on est revenu à plus bas
...
// softwareValueEncoderRight = softwareValueEncoderRight ; 

/*
    counterEncoder = rotKnobLeft.read();
    softwareValueEncoderLeft = counterEncoder / 8;

    counter2Encoder = rotKnobRight.read();
    softwareValueEncoderRight = counter2Encoder / 8;
*/
...
// BLACK05 softwareValueEncoderLeft est le rotknobLeft 


/*
    if (softwareValueEncoderLeft > SizeTransfo - 1)
    {
        setCounterLeft(0);
        // TODO BLACK05 modifier 
    }

    if (softwareValueEncoderLeft < 0)
    {
...
*/
    ...
    if (softwareValueEncoderLeft != oldCounterLeft)
    {
     ...
     softwareValueEncoderRight = Transfo[softwareValueEncoderLeft][4];
       ...
       // rotKnobRight.write(Transfo[softwareValueEncoderLeft][4] * 8);
       // counter2Encoder = rotKnobRight.read();
       //  softwareValueEncoderRight = counter2Encoder / 8;
...
/*
    value2Step = softwareValueEncoderRight;
...
/*   BLACK05
    if (softwareValueEncoderRight != oldC2)
    {
        int newValue = (int)(oldC2 + ((softwareValueEncoderRight - oldC2) * rotaryStepSize));
...
*/
    ...
    // if ( softwareValueEncoderLeft != oldCounterLeft || softwareValueEncoderRight != oldCounterRight)
    // {
    //     timeToClear = true ;
