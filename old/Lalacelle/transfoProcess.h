...
*/
	...
	case 552: // verbose Serial
	{
		...
		case 8:
		{
			verboseSerial_timeStamp = !verboseSerial_timeStamp;
		}
		break;

		case 9:
		{
			verboseSerial_OrderGiven = !verboseSerial_OrderGiven ;
		}
		break;
...
case 615 :
	transposeMIDIPlayer = potVal2 ;
	break ; // transposeMIDIPlayer = 0

// nouveaux cut off minimum et maximum pour le midi PLAY 

case 616:
loCutMIDI = potVal2 ; 
	break;
case 617:
loBringToMIDI = potVal2 ; 
	break;
case 618:
hiCutMIDI = potVal2 ; 
	break;
case 619:
hiBringToMIDI = potVal2 ; 
	break;
...
{ // FLUX REC 2
	...
	if (statusFluxRecIn == 0)
	{
		...
		// toutes les notes played vont se retrouver entretemps dans la layer de Flux currentFluxMem
		// prévoir une limite de notes !!!! sinon pourrait tout enregistrer
		// NOUVEAUTE : on enregistre aussi les note OFF et on limite donc les notes ON 
		buffer_and_HARD_setCursor(0, 4);
		buffer_clearLine(4);
		buffer_and_HARD_print("REC2      ");
	}
	else if (statusFluxRecIn == 2)
	{
		// vient d'appuyer une deuxième fois
		// toutes les notes played vont se retrouver entretemps dans la layer de Flux currentFluxMem
		...
		for (unsigned int j = 0; j < taille; j++)
		{
			...
			// ICI !!! il faut travailler la recherche des 0x80 
...
case 621 : CHPinch = potVal2; break ;
case 622:	CHPinchSize = potVal2; break  ; 
case 623:	CCPinch = potVal2; break ; 
case 624:	CCPinchSize = potVal2 ; break ; 
case 625:	valuePinch = potVal2; break ; 
case 626 : 
{
	...
	for (byte i = 0; i < nbLayer; i++)
	{
		if (isLayerActive(i))
		{
			...
			tempNote.note = 10;
			tempNote.velocity = 10;
			tempNote.layer = i;
			tempNote.channel = layerChannel[i];
			tempNote.type = 0x01; // PINCH
			tempNote.startNote = nowMillis % layerLength[i];
			tempNote.endNote = tempNote.startNote + 1 ;
			tempNote.killnote = false;
			tempNote.thereIsAnEnd = true;

			tempNote.pinchCH = CHPinch ;
			tempNote.pinchCH_size = CHPinchSize ;
			tempNote.pinchCC = CCPinch ;
			tempNote.pinchCC_size = CCPinchSize ;
			tempNote.pinchValue = valuePinch ;
		...
		break;
			...
			/////////////////////////////////////////////////////////////////////////////////////
			// Version 23C02
			//  Narayana's Cows - Tom Johnson - Lalacelle en Normandie 
			// avec Gilbert Delor, Deborah Walker, Sebastien Roux, Angelo K, Nikos Kokolakis, Samuel Bore 
			/////////////////////////////////////////////////////////////////////////////////////

		case 627:     // NARAYANA's COWS 
		{

// test 
// addNewNotetoPileONCE(pointeurInitial, 0, 60,											 100, nowMillis, 0x90 );
						deleteEnfants(); 
						enfantsBecomeParents(); 

			Enfants[0][0] = ageInitialPremiereVache;
			Parents[0][0] = ageInitialPremiereVache;

			Enfants[0][1] = initialPitch;
			Parents[0][1] = initialPitch;
...
// transfomer ici ShowGenerationParents en envoi de notes

			pileNote tempNote;
//			tempNote = ZEROgetNoteFromPile(pInitial[i], j);

			long long noteMoment;
			noteMoment = nowMillis + 2000; // délai 2 secondes

			for (int i = 0; i < sizeTroupeauMax; i++)
			{
				if (Parents[i][0] != 0)
				{
					if (Parents[i][0] > ( maturite - 1 ) )
					{
						//	cout << "O";
						tempNote.channel = 0 ;
						tempNote.layer = 1 ;
						tempNote.note = Parents[i][1]; 

						if ( narayanaMode == 2) 
						{
							tempNote.channel = Parents[i][1] ;
							tempNote.note = 60 ; 
						}
						...
						{
							tempNote.channel = Parents[i][1];
							tempNote.note = 64 + Parents[i][1] ;
						}

						tempNote.timeToPlayON = noteMoment ; 
						// tempNote.type = 
						tempNote.velocity = 90 + rand()%10;
						addNewNotetoPileONCE(pointeurInitial, tempNote.channel, tempNote.note,
											 tempNote.velocity, tempNote.timeToPlayON, 0x90, i);
						addNewNotetoPileONCE(pointeurInitial, tempNote.channel, tempNote.note,
											 0, tempNote.timeToPlayON+ dureeVache-10 , 0x90, i);
						noteMoment = noteMoment + dureeVache ; 
					}
					else
					{
						//	cout << ".";
						...
						addNewNotetoPileONCE(pointeurInitial, tempNote.channel, tempNote.note,
											 tempNote.velocity, tempNote.timeToPlayON, 0x90, i);
						addNewNotetoPileONCE(pointeurInitial, tempNote.channel, tempNote.note,
											 0, tempNote.timeToPlayON + dureeVeau - 10, 0x90, i);
						noteMoment = noteMoment + dureeVeau;
					}
				}
			}
		}
		...
		//	{628, 1, 127, TRANSFO_SIMPLE_VALUE},	  // : NameTransfo = F("-initial pitch"); break ;  //	int initialPitch = 64;						 // should be 64 : E

case 628 : initialPitch = potVal2 ; break ; 

		//		{629, 1, 10, TRANSFO_SIMPLE_VALUE},	  // : NameTransfo = F("-scale"); break ;  //	const int tailleScale = 2;					 // should be 2
		...
		case 629 : 
		{
			...
			{
				tailleScale = 3 ; 
				scale[0] = -1 ; scale[1] = -2 ; scale[2] = 2 ; 
			}

			if (potVal2 == 4)    // TB pour le jeu en channel ! partir de note = 0 ! 
			{
				tailleScale = 1;
				scale[0] = -1;
							}
		...
		break ; 
												  ...
												  // {630, 1, 127, TRANSFO_SIMPLE_VALUE}, // : NameTransfo = F("-scale2"); break ;  //	int scale[tailleScale] = {1, 3};			 // should be 1,3
	
	case 630 : 
	{narayanaMode = potVal2 ;
	
	if ( narayanaMode == 1 )
	{
		// initialisation classique
		  initialPitch = 64;			 // should be 64 : E
		  tailleScale = 2;			 // should be 2
		scale[0] = 1 ; scale[1] = 3 ;  	 // should be 1,3
		 maturite = 4;				 // should be 4
		 menopause = 1000;			 // should be infinite
		 mort = 1000;				 // should be infinite
		 ageInitialPremiereVache = 4; // should be 4
		 taillePortee = 1;			 // nombre de veaux qui naissent en même temps : should be 1
		mortaliteFortuite = 0;		 // en % les veaux et vaches qui meurrent aléatoirement : should be 0

		 Naranaya_BPM = 200;
		 dureeVache = (60 * 1000) / Naranaya_BPM;
		 ratioVeau = 2;
		 dureeVeau = dureeVache / ratioVeau; // should be 2
	}
	else 
	{
	  initialPitch = 0;			 // channel 0
		tailleScale = 1;			 // should be 2
		scale[0] = -1 ;			 // should be 1,3
		maturite = 4;				 // should be 4
		menopause = 1000;			 // should be infinite
		mort = 1000;				 // should be infinite
		ageInitialPremiereVache = 4; // should be 4
...
case 635 : taillePortee = potVal2 ; break ; 
...
case 636 : mortaliteFortuite = potVal2 ; break ; 

//				{637, 1, 300, TRANSFO_SIMPLE_VALUE}, // : NameTransfo = F("-bpm"); break ;  //	long Naranaya_BPM = 200;
case 637 : 
{
	Naranaya_BPM = potVal2;
	dureeVache = (60 * 1000) / Naranaya_BPM;
	dureeVeau = dureeVache / ratioVeau; 
}
break ; 
