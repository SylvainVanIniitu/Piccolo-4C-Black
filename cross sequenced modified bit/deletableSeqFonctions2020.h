...
////////////////////////////////////////
// BLACK08 : mise à jour du code corrigé de dec 2020,
// retrouvé sur le PC à Gaurain
// en cherchant la date 1/12/2020 !
////////////////////////////////////////

int processReturn(int b)
{
    // FloorSeq : valeur négative où les chiffres deviennent
    // des références aux séquences
    // pour débuter -100, mais peut être -2000 

    if (b <= floorSeq )
    {
        int a;
        a = giveNextValue(sequence[-(b - floorSeq )]);
        return a;
    }
    else
    {
...
int giveNextValue(sequenceStruct(&tab), long compteurThat)
{
    ...
    tab.compteur++;   // BLACK08 !!! 
    // faire gaffe qu'on ne fasse pas deux fois l'addition !
...
String giveStringOfSequence(byte s)
{
	...
	while (i < sequence[s].tailleSeq)
	{
        ...
        {
            ajout+= "=seq" ;
            ajout += String(-sequence[s].tableauSeq[i]+floorSeq);
