...
boolean hasSymmetricAxe(int gammeActuelle, int ModeActuel);
	...
	case 12: // MULTIPLIER de mélodie = MELTIPLY
	{
		...
		for (byte i = 0; i < nbLayer; i++)
		{
			...
			if (isLayerActive(i))
			{
				...
				calcultime = (unsigned long)layerLength[i] * (unsigned long)potVal2;
				calcultime = calcultime / (unsigned long)potVal3;
				layerLength[i] = (unsigned int)calcultime;
			...
			if (isLayerActive(i))
			{
				...
				{ //
						...
						if ((tempNote.note != 0) && (tempNote.startNote < foldtime) && (tempNote.endNote < foldtime))
					{
						...
						tempNote2.endNote = (unsigned int)((long)foldtime + ((long)foldtime - (long)tempNote.startNote));
						// melodyMatrixTime[i][j_35][MMTendnote] = constrain ( melodyMatrixTime[i][j_35][MMTendnote], 0, layerLength[i] )  ;

						tempNote2.startNote = (unsigned int)((long)foldtime + ((long)foldtime - (long)tempNote.endNote));
		...
		for (byte i = 0; i < nbLayer; i++)
		{
			...
			if (isLayerActive(i))
			{
				...
				{ // va parcourir la mélodie pointée ( Layer 1 à 4 : potVal3 )
					...
					tempNote.startNote = (unsigned int)layerLength[i] - (unsigned int)calcultime;
...
*/
	...
	*/
	...
	case 63: // FRACTIONARY REPLICATE
	{
		...
		int potVal3 = (counter2EndUser / 16) + 1; // était auparavant 9
		int potVal2 = (counter2EndUser % 16) + 1;
	...
	*/
	...
	case 81: // SAVE TO SD
	{
		...
		if (myFile)
		{
			...
			buffer_and_HARD_print(F("Saving "));
			buffer_and_HARD_setCursor(0, 1);
			buffer_and_HARD_print(fileString);
		}

		myFile.println("A // Version");
		// lire en première lettre un A indique que c'est ce modèle-ci de fichier

		for (byte i = 0; i < nbLayer; i++)
		{
			myFile.print(F("LAYER"));
			myFile.println(String(i + 1));

			myFile.print(String(layerLength[i]));
			myFile.print(F("\t"));
			myFile.println(F("//LENGTH(msec)"));
			myFile.print(String(fidelityLayer_[i]));
			myFile.print(F("\t"));
			myFile.println(F("//RANDOM(%)"));
			...
			for (byte k = 0; k < 3; k++)
			{
				...
				for (unsigned int j = 0; j < taille; j++)
				{
					...
					// myFile.print(String(melodyMatrix[i][j][k]));
					if (j < (taille - 1))
					{
						myFile.print(F("\t")); // prints a TAB
					}
					else
					{
						myFile.print(F("\t")); // prints a TAB
						...
						{
						case 0:
							myFile.println(F("// NOTE"));
							break;
						case 1:
							myFile.println(F("// VELOCITY"));
							break;
						case 2:
			...
			for (byte k = 0; k < 2; k++)
			{
				...
				for (unsigned int j = 0; j < taille; j++)
				{
					...
					// myFile.print(String(melodyMatrixTime[i][j][k]));
					if (j < (taille - 1))
					{
						myFile.print(F("\t"));
					}
		...
		myFile.println(F("you can experiment with layer lengths though !"));
		myFile.println(F("notes, velocity are from 0-127 ; data is a midi channel from 0-15 "));
		myFile.println(F("length and time is from 0-32000"));
		myFile.println(F("fidelity is 0-100"));
	...
	{ // LOAD FROM SD
		...
		// transplantation du sketch 115ReadSDTryOut
		...
		/*
	// à la limite on peut laisser tomber cette ouverture / fermeture
	...
	if (myFile) {
	u8x8.clear();
	buffer_and_HARD_setCursor(1, 0);
	buffer_and_HARD_print("File OPEN");
	//  delay(1000);
	else
	// if the file didn't open, print an error:
	u8x8.clear();
	buffer_and_HARD_setCursor(1, 0);
	buffer_and_HARD_print("ERROR OPENING");
	// delay(1000);
	...
	*/
		...
		u8x8.clear();
		buffer_and_HARD_setCursor(0, 0);
		buffer_and_HARD_print(F("Version:"));
		buffer_and_HARD_print(ligne.substring(0, 1));
		// delay(1000);
		myFile.read();

		for (byte i = 0; i < nbLayer; i++)
		{ // Va lire les 4 layers

			ligne = myFile.readStringUntil('\r');
			buffer_and_HARD_setCursor(0, 0);

			buffer_and_HARD_print(F("layer:"));
			...
			buffer_and_HARD_print(F("duree:"));
			buffer_and_HARD_print(ligne);

			layerLength[i] = ligne.toInt();
			...
			ligne = myFile.readStringUntil('\t');
			buffer_and_HARD_print(F("fidelite:"));
			buffer_and_HARD_print(ligne);

			fidelityLayer_[i] = ligne.toInt();
			...
			for (byte k = 0; k < 3; k++)
			{
				...
				// Va lire 3 lignes d'infos sur les notes, puis 2 sur le timing
				for (unsigned int j = 0; j < taille; j++)
				{
					ligne = myFile.readStringUntil('\t');
					buffer_and_HARD_setCursor(0, 0);
					...
					buffer_and_HARD_print(k);
					buffer_and_HARD_print(F("]="));
					buffer_and_HARD_print(ligne);

					// melodyMatrix[i][j][k] = ligne.toInt();
			...
			for (byte k = 0; k < 2; k++)
			{
				...
				for (unsigned int j = 0; j < taille; j++)
				{
					...
					buffer_and_HARD_print(k);
					buffer_and_HARD_print(F("]="));
					buffer_and_HARD_print(ligne.substring(0, 5));

					// melodyMatrixTime[i][j][k] = ligne.toInt();
					// TODO DEBUG à refaire complètement !
	...
	{ // LOAD & PLAY MIDI
		...
		buffer_and_HARD_print(F("***MIDI DONE***"));
		// delay(100);
	}
	break;
...
*/
	...
	*/
	...
	{ // GENERATE BPM
		...
		for (byte i = 0; i < nbLayer; i++)
		{
			...
			if (isLayerActive(i))
			{
				...
				layerLength[i] = (unsigned int)((float)60000.0 / (float)potVal2);
			...
			if (isLayerActive(i))
			{
				...
				{ // va parcourir la mélodie pointée ( Layer 1 à 4 : potVal3 )
					...
					if (tempNote.note != 0)
					{
						...
						tempNote.endNote = (unsigned int)NoteEnd2D.x;
	...
	break;
		...
		// boolean actif ; // une fois "ajouté" à la pile, il devient actif.
		// } ;
		//
		...
		for (byte i = 0; i < nbLayer; i++)
		{
			if (isLayerActive(i))
			{
				...
				for (unsigned int j = 0; j < taille; j++)
				{
					...
					while ((unsigned int)calculatedStart > layerLength[i])
					{
						calculatedStart -= layerLength[i];
					}
					while ((unsigned int)calculatedEnd > layerLength[i])
					{
	...
	case 248: // modulo layer Eclipse
	{
		...
		case (2):
		{
			...
			if (potVal2 < Transfo[counterEndUser][2] / 2)
			{
	...
	case 291:
	{
		...
		if (myFile.isOpen())
		{
			...
			// bool ren = myFile.rename(sd.vwd(), charmant );
			myFile.rename(sd.vwd(), charmant);
...
*/
	...
	case 346:
	{
		// pitchableLayer[potVal2 - 1] = !pitchableLayer[potVal2 - 1];
		for (byte i = 0; i < nbLayer; i++)
		{
			if (isLayerActive(i))
			{
	...
	break;
		...
		// 	{385, 0, 0, TRANSFO_NO_VALUE},	 // Revert : à l'écrevisse
		// 	{386, 0, 0, TRANSFO_NO_VALUE},	 // Reflet : modifie le pitch en fc du plus élevé et plus bas
...
*/
	...
	case 529: // d'après 377
	{
		...
		{
			//	Serial.print("titles.txt exists.");
		}
		else
		{
			//	Serial.print("titles.txt doesnt exists. creating one");

			if (sd.begin())
			{
				myFile.open("titles.txt", FILE_WRITE);
	...
	{ // midi ratio multiplier
		...
		if (potVal2 > 1)
		{
			RatioDeltaToMsec = RatioDeltaToMsec * potVal2;
		}
	...
	// jusqu'à un max de potVal2 - 1 
		...
		for (byte i = 0; i < nbLayer; i++)
		{
			...
			if (isLayerActive(i))
			{
				...
				{
					compteur++ ; 
					if ( random(100) < pourcentageDecalagem )
					 {
					tempNote = ZEROgetNoteFromPile(pInitial[i], j);
					KeepDuree = getDuree(tempNote);
