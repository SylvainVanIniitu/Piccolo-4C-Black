...
// DONE
// 2C26
// fix Variations OFF 2C26

// 2C19 mise au point d'un buffer vidéo

//2C17 cannibalisation de Dende

// CC pour Strymon ou behringer
// utiliser 1 2 3 plutot que 0 1 2 - sinon pas End User friendly
// save All characteristics

// 2C12 Kannon2
// 1. kompakter N DONE
// 2. simplification de la numérotation, abandon du double numéro de fichier, numéro de programme, pour un numéro unique
// 3. Steps in CC REc
// 4. Repeater + BitMap
// 5. modeActuelle et gammeActuelle deviennent par layer ( 73 lieux de modif )

// 2C02 Empathy3
// 1. ajout Matrix -> Seq ( version quick )
// 2. ajout option CC / pitch shift / pitch SET

// 2C01 Empathy
// TODO :
// 1. réécrier Cell avec CH-N -> CH-N / DONE
// 2. intégrer Save/Load de Cell à saveLoadLocal DONE
// 3. suppression SaveCell / DONE (en partie) DONE
// 4. save All characteristics
// 5. pile pour CCMod Events Sequences ! DONE
// 6. développement des nouveaux modes de Seq et intégration à diverses fonctions !  début avec pitch MicroSampler !

// 1C99 6Cellos : Save Local en grande partie réalisé
// 1. terminer load/save DONE
// 2. faire load partiels DONE
// 3. suppression des 4 fichier Save différents DONE

// 1C97 Schneider :
// 1. élimination complète des melodyMatrix DONE
// 2. unpitchable Channel DONE
// 3. unpitchable Layer DONE
// 4. eraser note DONE
// GRANDE SIMPLIFICATION DE CREATION DE STRING POUR AFFICHAGE : supprimé !
// 5. live swing
// 6. Automation Recorder
// 7. load save

// 1C91 Regensburg : passage capital des array vers les piles de notes,
// pour la rapidité, l'efficacité mémoire
// passage simultané de array pour notes vers struct !

// première chose à développer : Inspectah Note
// spécifier layer
// on choisit la note dans la pile
// on en connaît toutes les données
// ensuite : Delete
// ensuite : Insert
// ensuite : Swap
// ensuite : kill pile
// ensuite : duplicate pile

// ajouter le fait que le menu retienne la valeur précédemment choisie

// DETERMINER LES PRIORITES A CODER POUR CES VACANCES

// SAVE LOAD, sans AUCUN doute
// PILE NOTE
// UNDO
// MULTI MARKOV
// DELAY SIMPLE
...
// TODO REgression
// affichage MIDI parallele pour CCMOD
// Affichage du CCMod en cours, comme pour Sequence et Events
...
U8X8_SSD1306_128X64_NONAME_SW_I2C u8x8(/* clock=*/18, /* data=*/19, /* reset=*/U8X8_PIN_NONE); // OLEDs without Reset of the Display
...
int pinRotaryEncoder7A = 15; // MENU2
int pinRotaryEncoder7B = 14;

int pinRotaryEncoder8A = 9; // PITCH
int pinRotaryEncoder8B = 6;

int pinRotaryEncoder9A = 25; // MENU1
int pinRotaryEncoder9B = 24;

// BOUTONS

int pinClickButton1 = 32;  // Delay 1
int pinClickButton2 = 16;  // CH3
int pinClickButton3 = 17;  // CH4
int pinClickButton4 = 20;  // CH5
int pinClickButton5 = 21;  // CH6
int pinClickButton6 = 22;  // OK BUTTON
int pinClickButton7 = 23;  // SHIFT
int pinClickButton8 = 33;  // Delay 2
int pinClickButton9 = 34;  // Delay 3
int pinClickButton10 = 38; // Delay 4
int pinClickButton11 = 39; // CH1
int pinClickButton12 = 13; // CH2

// TODO : il faut réécrire load/save

Encoder rotKnob(pinRotaryEncoder1A, pinRotaryEncoder1B);
Bounce myButtonRE1(pinPushRE1, 10); // bounce sur 20 msec
Encoder rotKnob2(pinRotaryEncoder2A, pinRotaryEncoder2B);
Bounce myButtonRE2(pinPushRE2, 10); // bounce sur 20 msec
Encoder rotKnob3(pinRotaryEncoder3A, pinRotaryEncoder3B);
Bounce myButtonRE3(pinPushRE3, 10); // bounce sur 20 msec
Encoder rotKnob4(pinRotaryEncoder4A, pinRotaryEncoder4B);
Bounce myButtonRE4(pinPushRE4, 10); // bounce sur 20 msec
Encoder rotKnob5(pinRotaryEncoder5A, pinRotaryEncoder5B);
Bounce myButtonRE5(pinPushRE5, 10); // bounce sur 20 msec
Encoder rotKnob6(pinRotaryEncoder6A, pinRotaryEncoder6B);
Bounce myButtonRE6(pinPushRE6, 10); // bounce sur 20 msec
Encoder rotKnob7(pinRotaryEncoder7A, pinRotaryEncoder7B);

// Bounce myButtonRE7(pinPushRE7,20) ; // bounce sur 20 msec
Encoder rotKnob8(pinRotaryEncoder8A, pinRotaryEncoder8B);
// Bounce myButtonRE9(pinPushRE8,20) ; // bounce sur 20 msec
Encoder rotKnob9(pinRotaryEncoder9A, pinRotaryEncoder9B);
// Bounce myButtonRE9(pinPushRE9,20) ; // bounce sur 20 msec

Bounce myButton(pinPushRE1, 10);

Bounce myButtonDelay1(pinClickButton1, 10);  // Delay 1
Bounce myButtonDelay2(pinClickButton8, 10);  // Delay 2
Bounce myButtonDelay3(pinClickButton9, 10);  // Delay 3
Bounce myButtonDelay4(pinClickButton10, 10); // Delay 4

Bounce myButtonChannel1(pinClickButton11, 10);
Bounce myButtonChannel2(pinClickButton12, 10);
Bounce myButtonChannel3(pinClickButton2, 10);
Bounce myButtonChannel4(pinClickButton3, 10);
Bounce myButtonChannel5(pinClickButton4, 10);
Bounce myButtonChannel6(pinClickButton5, 10);

Bounce myButtonOK(pinClickButton6, 10);
Bounce myButtonSHIFT(pinClickButton7, 10);
...
void setup()
{
    ...
    // setup des boutons d'encodeurs rotatifs

    pinMode(pinPushRE1, INPUT);
    pinMode(pinPushRE1, INPUT_PULLUP);

    digitalWrite(pinPushRE1, HIGH);

    pinMode(pinPushRE2, INPUT);
    digitalWrite(pinPushRE2, HIGH);

    pinMode(pinPushRE3, INPUT);
    digitalWrite(pinPushRE3, HIGH);

    pinMode(pinPushRE4, INPUT);
    digitalWrite(pinPushRE4, HIGH);

    pinMode(pinPushRE5, INPUT);
    digitalWrite(pinPushRE5, HIGH);

    pinMode(pinPushRE6, INPUT);
    digitalWrite(pinPushRE6, HIGH);

    // setup des boutons simples

    pinMode(pinClickButton1, INPUT);
    digitalWrite(pinClickButton1, LOW);

    pinMode(pinClickButton2, INPUT);
    digitalWrite(pinClickButton2, LOW);

    pinMode(pinClickButton3, INPUT);
    digitalWrite(pinClickButton3, LOW);

    pinMode(pinClickButton4, INPUT);
    digitalWrite(pinClickButton4, LOW);

    pinMode(pinClickButton5, INPUT);
    digitalWrite(pinClickButton5, LOW);

    pinMode(pinClickButton6, INPUT);
    digitalWrite(pinClickButton6, LOW);

    pinMode(pinClickButton7, INPUT);
    digitalWrite(pinClickButton7, LOW);

    pinMode(pinClickButton8, INPUT);
    digitalWrite(pinClickButton8, LOW);

    pinMode(pinClickButton9, INPUT);
    digitalWrite(pinClickButton9, LOW);

    pinMode(pinClickButton10, INPUT);
    digitalWrite(pinClickButton10, LOW);

    pinMode(pinClickButton11, INPUT);
    digitalWrite(pinClickButton11, LOW);

    pinMode(pinClickButton12, INPUT);
    digitalWrite(pinClickButton12, LOW);
    ...
    // laPile.type = 0;     // n'est ni note ON, ni note OFF, ni CC.  rien en fait
    // laPile.killnote = false;

    // pointeurInitial = &laPile;

    rotKnob6.write(50 * 4); // introduit une valeur de LFOA de 5.0 secondes
...
buffer_and_HARD_print(end - start);

  start = micros();
for (int i = 0; i < 1000; i++)
{
    printRot2("O", 16, 400, 1);
}
  end = micros();
...
void loop()
{
    ...
    myButtonDelay1.update();
    myButtonDelay2.update();
    myButtonDelay3.update();
    myButtonDelay4.update();

    myButtonChannel1.update();
    myButtonChannel2.update();
    myButtonChannel3.update();
    myButtonChannel4.update();
    myButtonChannel5.update();
    myButtonChannel6.update();
    ...
    // switches[0] = !digitalRead(44) ;
    // switches[1] = !digitalRead(42) ;
    // switches[2] = !digitalRead(40) ;
    // switches[3] = !digitalRead(38) ;
    // TODO DEBUG utiliser des interrupts quand on sera sur le Teensy !
    ...
    //     }
    // }
    // TODO on devrait ici mettre une option pour indique si c'est la transformation 20 qui a la main, ou les switches

    // DEBUG NOTE remplacer par un interrupt
    //
    // NOTE j'avais essayé de condenser en une ligne mais ça a foiré

    // for (byte i = 0 ; i < 4 ; i++) {
    //     muteLayer = muteLayer + (switches[!digitalRead(38+(i*2)+1 )]<<i)   ;
    // }

    // qd 39, 41, 45, 45 valent 1 alors MUTE
    // if switches[2]==1 {muteLayer=muteLayer}

    // switch1UP = ( switches[0]==0? true : false ) ;
    // switch1DOWN = ( switches[1]==0? true : false ) ;
    // switch2UP = ( switches[2]==0? true : false ) ;
    // switch2DOWN = ( switches[3]==0? true : false ) ;
    // switch3UP = ( switches[4]==0? true : false ) ;
    // switch3DOWN = ( switches[5]==0? true : false ) ;
    // switch4UP = ( switches[6]==0? true : false ) ;
    // switch4DOWN = ( switches[7]==0? true : false ) ;
    // muteLayer = muteLayer ^ ( 1 << i ) ; // si le bit est 0 deviendra 1 et s'il est 1 deviendra 0
    // i de 0 à 4
    // pas vraiment besoin de nouvelles variables pour les switch en fait
    // si je retiens
    ...
    // partie enregistrement MIDI

    //if ( record  &&   MIDI.read(0)  )
...
*/
    ...
    */
    ...
    // if (counterspeed % 5 == 0)
    // tentative de réduire la fréquence de lecture et donc d'affichage du menu !
    {

        // checkButton8and9PushButtonsRotaryEncoders(); // vérifie les pushbuttons d'encodeurs rotatifs

        /* affichage des infos importantes

    ??? : un signe pour dire enregistre RECORD
    */

        // potVal = map(potVal, 0, 1023, 1, SizeTransfo + 1 ); // s'adapte au nb de fonctions activées
        ...
        counter2Encoder = rotKnob7.read();
        counter2EndUser = counter2Encoder / 8;

        pageEncoder = rotKnob8.read();
    ...
    if (pageEndUser > numberMaxPageStepz)
    {
        ...
        rotKnob8.write(numberMaxPageStepz * 8);
        pageEncoder = rotKnob8.read();
        pageEndUser = pageEncoder / 8;
        currentPageStepz = pageEndUser;
    }
    ...
    if (counterEndUser != oldcounter)
    {
        ...
        rotKnob7.write(Transfo[counterEndUser][4] * 8);
        counter2Encoder = rotKnob7.read();
        counter2EndUser = counter2Encoder / 8;
            ...
            Evenement[CurrentEvenement].type == 2)
    {
        ...
        if (counter2EndUser > Transfo[LineCallingTransfo(Evenement[CurrentEvenement].which)][2])
        {
            ...
            // counter2Encoder = counter2EndUser * 4  ;
            // rotKnob2.write(counter2Encoder ) ;
            setCounter2(Transfo[LineCallingTransfo(Evenement[CurrentEvenement].which)][2]);
            oldC2 = counter2EndUser;
        }
        ...
        if (counter2EndUser < Transfo[LineCallingTransfo(Evenement[CurrentEvenement].which)][1])
        {
            ...
            // counter2Encoder = counter2EndUser * 4  ;
            // rotKnob2.write(counter2Encoder ) ;
            setCounter2(Transfo[LineCallingTransfo(Evenement[CurrentEvenement].which)][1]);
            oldC2 = counter2EndUser;
        }
    }
...

    {
        if (counter2EndUser > Transfo[counterEndUser][2])
        {
            ...
            // counter2Encoder = counter2EndUser * 4  ;
            // rotKnob2.write(counter2Encoder ) ;
            setCounter2(Transfo[counterEndUser][2]);
            oldC2 = counter2EndUser;
        }
        ...
        if (counter2EndUser < Transfo[counterEndUser][1])
        {
            ...
            // counter2Encoder = counter2EndUser * 4  ;
            // rotKnob2.write(counter2Encoder ) ;
            setCounter2(Transfo[counterEndUser][1]);
            oldC2 = counter2EndUser;
        }
    }
