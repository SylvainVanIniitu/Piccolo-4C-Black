...
//
boolean isNegativeSByte(byte const &thi);
boolean isSilentSByte(byte const &thi);
int valueSByte(byte const &thi);
byte makeSByte(int thi, boolean const &silent);
...
boolean isLayerActive(byte layer);
...
boolean isWinogradDeep(string &string1)
{
...
boolean isErdosDeep(string &string2)
{
...
boolean isAksak(string &string3)
{
...
boolean isOdd(int &cenombre)
{
	// détermine si est impair

	boolean odd = ((cenombre % 2) == 0 ? false : true);
...
void ReadVLVDelta()
{
	...
	do
	{
		VLVDelta = VLVDelta << iShiftDelta; // 8 ;
		readfromFile(1);
		...
		DureeDelta = DureeDelta << iShiftDuree;				 // 7 ;
		DureeDelta = DureeDelta + (mididouble & iDureeMask); // B01111111 ) ;
															 // } while (  ( mididouble & iMask2  ) == iMask2 && (myFile.available() == true) ) ; // tant que le bit 7 est 1 il faut continuer à lire.
	} while ((mididouble & iMask2) == iMask2);				 // tant que le bit 7 est 1 il faut continuer à lire.
...
// if ( ( nowMillis - lastStart ) < proxyPitch ) { pitch += proximityPitch  ; }
			...
			pileNote tempNote;
			tempNote.note = pitch;
			tempNote.MMnoteReallyPlayed = pitch;
	...
	if (statusFluxRecIn == 1 && (cmd_isolee) == 0x90)
	...
	*/
	...
	if (FunkyVel == true && cmd_isolee == 0x90) // 2C27
	{
		...
		{ // çàd on joue une note du MidiParallel
			add = random(0, FunkyVelValueMIDIPlayer);
		}
...
void playmidifile1_DISMISS(void)
{
	// Open serial communications and wait for port to open:

	rotKnob7.write(500); // valeur approximativement OK pour le départ
	...
	myFile.close();
	myFile = sd.open(midiname, FILE_READ);
	...
	//
	//     /* si x = le nb de ticks / quarter-note
	//     alors x * 4 = le nb de ticks par note
	//     et dans une minute il y a y BPM,
	//     soit BPM * x * 4 ticks / minute
	//     donc un tick = 60000 / ( BPM ¨* 4 * x ) msec
	//     et dès lors x ticks = 60000 / ( BPM * 4 ) msec
	//     */
	// } // else {
	...
	do
	{
		...
		do
		{
			...
			currentKnob2 = rotKnob7.read();
			// delay ( long ((( DureeDelta * 60000 ) / calculatedBPM ) / NbTicksQuarterNote) * (currentKnob2 / 50) );
			midiplayerdelay();
			...
			case (0x80):
			{
				...
				// 0kkkkkkk
				// 0vvvvvvv
				// quid représentation visuelle ( comme un clavier )
				// find lowest note
				// find highest note
				// calculate how many notes
				// calculate additive VLV duration
			}
			break;
			...
			case (0x90):
			{
				...
				if (nbNotesOn % 3 == 0)
				{
					...
					buffer_and_HARD_print(CurrentMIDIChunk);
					buffer_and_HARD_print(F(" "));
					buffer_and_HARD_print(nbNotesOn);
					buffer_and_HARD_print(F(" K"));
					buffer_and_HARD_print(currentKnob2);
					buffer_and_HARD_print(F(" CH"));
					buffer_and_HARD_print(midichannel); // Devra être adapté après certains CC
					buffer_and_HARD_print(F(" "));
				}
				//midiplayerdelay() ;
				PlaynoteOn2(exact1stbyte, byte(mididouble >> 8), byte(mididouble & B01111111), B10000000);

				// ajouter un switch qui décide si on fait le delay AVANT ou APRES note On ou OFF
			...
			{ // Program Change
				...
				// NE PAS UTILISER PLAY NOTE CAR n'a qu'un seule BYTE DE DATA, pas 2 !
			...
			default: // Running Status ???
				...
				case (0x80):
				{
					...
					// faut-il le delay avant ou après que la note ait été arrêté ???  à essayer !!!!
					// ********************************$
				}
				break;
...
boolean FileExists(string &thisFile)
{

	//   if (sd.exists(thisFile)) {
	//   return (true);
	// } else {
	// return (false) ;
	//  }
...
void deleteFile(string &thisFile)
{
	...
	File deleteFile;
	deleteFile = sd.open(thisFile, FILE_WRITE);
	deleteFile.remove();
	// A ADAPTER POUR LE PASSAGE SD vers SdFAT
}
...
{
	if (DureeDelta > CutOffDureeDelta)
	{
		DureeDelta = BringToDureeDelta;
	}
...
fract closestFraction(int a, int b, int maxSteps, boolean tryAllSteps)
{
...
subOP = note % 13 ;
	...
	switch ( subOP )
	{
	case 0 : sub = map ( sub, 0,127, 0, 31 ); break ;
	case 1 : sub = map ( sub, 0,127, 0, 31 ); break ;
	case 2 : sub = map ( sub, 0,127, 0, 31 ); break ;
	case 3 : sub = map ( sub, 0,127, 1, 15 ); break ;
	case 4 : sub = map ( sub, 0,127, 0, 15 ); break ;
	case 5 : sub = map ( sub, 0,127, 0, 99 ); break ;
	case 6 : sub = map ( sub, 0,127, 0,3 ); break ;
	case 7 : sub = map ( sub, 0,127, 0, 7 ); break ;
	case 8 : sub = map ( sub, 0,127, 0, 1 ); break ;
	case 9 : sub = map ( sub, 0,127, 0, 7 ); break ;
	case 10 : sub = map ( sub, 0,127, 0, 99 ); break ;
	case 11 : sub = map ( sub, 0,127, 0, 63 ); break ;
	case 12 : sub = map ( sub, 0,127, 0, 6); break ;
}
}

switch ( note )
	{
	case 52 : sub = map ( sub, 0,127, 0, 7); break ;
	case 53 : sub = map ( sub, 0,127, 0, 7); break ;
	case 54 : sub = map ( sub, 0,127, 0, 99 ); break ;
	case 55: sub = map ( sub, 0,127, 0, 99  ); break ;
	case 56: sub = map ( sub, 0,127, 0, 99 ); break ;
	case 57: sub = map ( sub, 0,127, 0, 99 ); break ;
	case 58 : sub = map ( sub, 0,127, 0,1 ); break ;
	case 59 : sub = map ( sub, 0,127, 0, 3 ); break ;
	case 60 : sub = map ( sub, 0,127, 0, 7 ); break ;
	case 61 : sub = map ( sub, 0,127, 0, 3 ); break ;
	case 62 : sub = map ( sub, 0,127, 0, 48 ); break ;
	case 63 : sub = map ( sub, 0,127, 0, 1 ); break ;
	case 64: sub = map ( sub, 0,127, 0, 12); break ;
		case 65 : sub = map ( sub, 0,127, 0, 1 ); break ;
	case 66 : sub = map ( sub, 0,127, 0, 99 ); break ;
	case 67 : sub = map ( sub, 0,127, 0, 99); break ;
	case 68: sub = map ( sub, 0,127, 0, 1 ); break ;
	case 69: sub = map ( sub, 0,127, 0, 1 ); break ;
	case 70: sub = map ( sub, 0,127, 0, 1 ); break ;
	case 71 : sub = map ( sub, 0,127, 0, 99); break ;
	case 72: sub = map ( sub, 0,127, 0, 99 ); break ;
	case 73 : sub = map ( sub, 0,127, 0, 99 ); break ;
	case 74 : sub = map ( sub, 0,127, 0, 99 ); break ;
	case 75 : sub = map ( sub, 0,127, 0, 99 ); break ;
	case 76 : sub = map ( sub, 0,127, 0, 99 ); break ;
	case 93: sub = map ( sub, 0,127, 0, 15); break ;
	default : break ;
}
...
subOP = note % 5 ;
	...
	switch ( subOP )
	{
	case 0 : sub = map ( sub, 0,127, 0, 1); break ;
	case 1 : sub = map ( sub, 0,127, 1, 7 ); break ;
	case 2 : sub = map ( sub, 0,127, 0, 15 ); break ;
	case 3 : sub = map ( sub, 0,127, 0, 7 ); break ;
	case 4 : sub = map ( sub, 0,127, 0, 3 ); break ;
}
}

switch ( note )
	{
	case 20 : sub = map ( sub, 0,127, 0, 7); break ;
	case 21 : sub = map ( sub, 0,127, 0, 99); break ;
	case 22 : sub = map ( sub, 0,127, 0, 99 ); break ;
	default : break ;
}
...
void randomizeNoteRedirect()
{
	...
	for (byte i = 0; i < 12; i++)
	{
		...
		// boolean currentStatus = isSilentSByte(noteRedirect[noteToWorkOn] );
		boolean currentStatus = false; // on ne garde pas silencieuses les notes qui le sont
...
void pileNote_actualizeTimeToPlay(pileNote &cetteNote)
{
	...
	if (drunk[i] && cetteNote.type != 0xB0)
	{
		...
		if (random(0, 2) == 0)
		{
			currentDrunkValue[i] += drunkStep[i];
		}
		else
		{
			currentDrunkValue[i] -= drunkStep[i];
		}
		if (currentDrunkValue[i] > drunkMax[i])
		{
			currentDrunkValue[i] = drunkMax[i];
		}
		if (currentDrunkValue[i] < drunkMin[i])
		{
			currentDrunkValue[i] = drunkMin[i];
		}
...
boolean isLayerEclipsed(byte layer)
{
...
boolean isLayerMuted(byte layer)
{
...
string afficheRatio(int valeur, byte maxNum, byte maxDen, boolean includeNegatif = false)
{
...
void setCounter2(int const &a)
{
	counter2EndUser = a;
	counter2Encoder = counter2EndUser * 8;
	rotKnob7.write(counter2Encoder);
}
...
// on peut imaginer qu'elle renvoie 0 ou 1 pour indiquer si overflow ou qqch du style, mais pas prioritaire
{
	if (pitchableChannel[cetteNote.channel] == true && pitchableLayer[cetteNote.layer] == true)
	{
		signed int newValue = (signed int)cetteNote.MMnoteReallyPlayed;
		newValue += (signed int)modification;
		newValue = limiterForPitch(newValue);
		cetteNote.MMnoteReallyPlayed = newValue;
	}
}
...
// se base sur le nb de Swing, mais fait d'office | >
{
	...
	if (iAB % 2 == 1)
	{
		tempA = iAB * lgAB;		  // donne le temps dans la grille parfaite avant startnote
		tempB = (iAB + 1) * lgAB; // donne le temps dans la grille parfaite après startnote
		float ftempC = ((float)tempB - (float)tempA) * (float)valueDynamicSwing[i] / (float)100.0;
		long tempC = (long)tempA + (long)ftempC;
...
// va effacer les tabulations, espaces, et supprimer ce qui suit // en commentaire
{
	...
	ligne.trim();
	int trouve;
	trouve = ligne.indexOf("//");
	ligne = ligne.substring(0, trouve);
}
