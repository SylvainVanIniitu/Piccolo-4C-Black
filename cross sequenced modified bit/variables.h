...
const string idRecordNoteRedirect_End = "// NOTE REDIRECT END -------------------------------------";
...
boolean baroccoLayer[nbLayer] = {false, false, false, false};
...
boolean vel2Vol[16] = {false};

boolean delayLayer[nbLayer] = {false, false, false, false};
...
boolean tempoStarted = false;

double timeZero = 0;

boolean SDCardPresente = false;
...
int counterEndUser(0);
int counter2Encoder(0);
int counter2EndUser(0);
int pageEncoder(0);
int pageEndUser(0);
int oldcounter = 0;
int oldcounter2 = 0;
...
boolean modeConstipe = false; // NOTE DEBUG prévoir une transfo pour on/off
...
// 1 = + 1
// 2 = -1
boolean harmonizeLayer[nbLayer] = {false};
...
boolean specialDebug = false; // met le Serial à 115200 et envoie note et VLVDelta
...
// 2C22 boolean parser = false; // si false : va jouer les notes, sinon va faire un display sur le monitor
...
const string barremilieu = "|"; // barre droite
...
// boolean pressed1, pressed2, pressed3 = false ; // indique qu'on a pressé sur le bouton - permet d'éviter l'utilisation de delay
boolean automodification = false;  // si devient vrai, tous les automodificationtime, il y a aura modif automatique
boolean Daubymodification = false; // si devient vrai, lance des modifications à chaque itération pour la TX81Z
...
boolean RECtiming;     // indique si est en train d'enregistrer la durée
boolean record = true; // indique si on est en train d'enregistrer une mélodie. 2C28 TRUE d'office
...
boolean Rfirsttime[printRotSlots];
...
byte iShiftDelta = 8;
byte iShiftDuree = 7;
byte iDureeMask = 0xB01111111;
byte iMask2 = 0xB10000000;
...
boolean playParallelMidiON = false;
boolean MUSTreadHeader = true;
...
boolean switches[8]; // valeur des différents switches  0 et 1 sont pour le premier : UP et DOWN ; 2 et 3 pour le 2nd ; etc
...
boolean dontRotateLength = false;
...
boolean sendCCtoTX81Z = false;
boolean sendCCtoSHERMAN = false;
...
fract fraction;
boolean showFract = true; // si true va montrer tous les temps en fractions !
